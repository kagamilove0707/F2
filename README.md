...
またlet rec式（`let rec 名前 式1 in 式2`）で再帰的な関数を束縛できるのです＞ω＜

```
(13)# let rec fact = fun x -> if x <= 0 then 1 else x * fact (x - 1) in fact 5
...
デフォルトで定義されている関数です(｀・ω・´)　どうにか階上や竹内関数を実装できるようになったのです＞＜
